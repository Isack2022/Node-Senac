...
app.put("/users/update/:id", verfToken, (req, res) => {
    //pegando a senha que foi enviada pelo usuário(Front)
    let sh = req.body.senha;
...
/*
Verificar se o usuário tem um tonken válido. Caso tenha signficar que ele logou e, portanto,
pode atualizar os dados
*/
function verfToken(req,res,next){
    const token_enviado = req.headers.api_key;
    if(!token_enviado){
        return res.status(401).send({output:"Acess Denied"});
    }
    jwt.verify(token_enviado,process.env.JWT_KEY,(erro,result)=>{
        if(erro){
            return res.status(500).send({output: `Internal erro to valid token`});
